...
"resources/soratomo.png"]);f.preload(["resources/star_effect.png","resources/heart_effect.png","resources/explode.png","resources/isse-no-de.png","resources/kamifubuki.png"]);f.preload(["resources/mame.png"]);f.preload(["resources/fullscreen.png","resources/volume.png","resources/Achan_icon.png","resources/sorach_balloon.png"]);STAGES.forEach(function(a){a.card&&f.preload("http://i.ytimg.com/vi/"+a.card.id+"/default.jpg")});PICK_UP_CARDS_ON_CLEAR.forEach(function(a){f.preload("http://i.ytimg.com/vi/"+
a.id+"/default.jpg")});PICK_UP_CARDS_ON_GAMEOVER.forEach(function(a){f.preload("http://i.ytimg.com/vi/"+a.id+"/default.jpg")});var M=Class.create(Scene,{initialize:function(){Scene.call(this);var a=f.assets["resources/\u3061\u3087\u3063\u3068\u3057\u305f\u3089\u6674\u308c.mp3"].clone();a.play();a.volume=w/3*.25;new ButtonVanishBugFix(this);var b=new Sprite(384,512);b.image=f.assets["resources/bg_pattern1_aozora.jpg"];b.moveTo(0,0);this.addChild(b);b=new Sprite(320,128);b.image=f.assets["resources/title.png"];
...
a+=this.amax_mode?"\u305d\u3057\u3066\u3048\u30fc\u307e\u3063\u304f\u3059\uff01\uff01":"";a=b==c?a+"\u30ab\u30f3\u30da\u30ad\u3067\u3059\u30fc\uff01\uff01\uff01\n":0==b?a+"\u306a\uff5e\u3093\u3067\u306a\u3093\u3060\u3044\u3063\uff01\uff01\n":a+"\n";break;case m.STATE.GAMEOVER:a+="\u7d50\u679c\uff1a(\u0e51>\u11ba<)\ud83d\udca6\n"}window.open("http://twitter.com/intent/tweet?text="+encodeURI(a)+"&hashtags=\u3068\u304d\u306e\u305d\u3089&url="+encodeURI("https://likeslug.github.io/hopsteptokinosora/game"))}}),
...
a.width/2,a.y+a.height/2),a.parentNode.effectLayer);a.parentNode.removeChild(a)});512<this.y&&this.parentNode.removeChild(this)}}),Z=Class.create(Group,{initialize:function(a){Group.call(this);this.width=384*.8;this.height=100;var b=new Entity;b.width=384*.8;b.height=100;b.backgroundColor="#FFF8";this.addChild(b);var c=new Sprite(120,90);c.image=f.assets["http://i.ytimg.com/vi/"+a.id+"/default.jpg"];c.moveTo(10,(b.height-c.height)/2);this.addChild(c);var d=new Label(a.caption);d.width=b.width-c.width-
30;d.height=c.height/2;d.font="16px "+y;d.moveTo(c.x+c.width+10,c.y);this.addChild(d);b=new Label(a.title);b.width=d.width;b.font="10px "+y;b.moveTo(c.x+c.width+10,d.y+d.height);this.addChild(b);this.on("touchend",function(b){window.open("http://www.youtube.com/watch?v="+a.id+(a.sec?"#t="+Math.floor(a.sec/60)+"m"+a.sec%60+"s":""))})}}),X=Class.create({initialize:function(a,b){this.sora=a;this.code=b;this.done_idx=0},update:function(){var a=this.code;if(a){var b=this.sora;b=Math.floor((b.x+(b.width-
